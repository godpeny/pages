...
$$ A \setminus B $$
Remove elements of the second set($B$) from the first set.
For example,
$$
u = \{1,2,3,4\}, \quad v = \{2,4\} \\[5pt]
u \setminus v = \{1,3\}
$$
...
### Radon's theorem
$d$차원 유클리드 공간 $\mathbb{R}^d$에 있는 임의의 $d + 2$개의 점들의 집합은, 서로소(disjoint)인 두 개의 부분집합으로 분할될 수 있으며, 이때 두 부분집합의 볼록 껍질(Convex Hull)은 반드시 교집합을 가집니다.

쉽게 말해 "차원 공간에 점이 충분히 많으면, 이 점들을 두 그룹으로 잘 나누어서 각 그룹이 차지하는 영역(볼록 껍질)이 서로 겹치게 만들 수 있다"는 정리입니다.

#### Example of Radon's theorem
<img src="images/blog0_radon_theorem.png" alt="Example of Radon's theorem" width="400"/>     

평면($d = 2$)을 예로 들면, 필요한 점의 개수는 $2 + 2 = 4$개입니다. 평면에 평범하게 4개의 점을 찍으면 다음과 같은 두 가지 경우의 수만 존재하며, 어떤 경우든 두 그룹으로 쪼개어 겹치게 만들 수 있습니다.

- 삼각형과 그 내부의 점 (3+1 분할): 세 점이 큰 삼각형을 이루고 한 점이 그 내부에 있는 경우입니다. [삼각형(세 점의 볼록 껍질)]과 [내부의 점 하나]로 나누면, 내부의 점 자체가 교차점이 됩니다.
- 볼록 사각형 (2+2 분할): 네 점이 볼록한 사각형을 이루는 경우입니다. 이때는 대각선 방향으로 마주 보는 점끼리 짝을 지어 [두 점을 잇는 선분]과 [나머지 두 점을 잇는 선분]으로 나눕니다. 두 선분(볼록 껍질)은 사각형 내부에서 반드시 교차하게 됩니다.

#### Vapnik–Chervonenkis dimension (VC 차원)
머신러닝에서 모델의 복잡도나 학습 능력을 측정하는 VC 차원을 계산할 때 사용됩니다. $d$차원 공간에서 점들을 직선이나 평면으로 완벽히 분리할 수 있는 최대 점의 개수가 $d+1$개라는 것을 증명할 때 라돈 정리가 사용됩니다.  
($d+2$개부터는 라돈 분할 때문에 평면으로 두 그룹을 떼어놓을 수 없기 때문입니다.)

### Convex Set
볼록 집합이란, 집합 내의 임의의 두 점을 연결한 선분이 항상 그 집합 내부에만 존재하는 형태를 뜻합니다. (즉, 두 점을 연결한 선분이 내부에 있다.)

<b> 볼록 집합인 것들 (통통한 모양) </b >  
꽉 찬 원, 정삼각형, 직사각형, 하트가 아닌 그냥 둥근 달 모양

특징: 이 도형들 안에서는 아무 데나 점 2개를 찍고 자로 곧게 선을 그어도, 그 선이 도형 밖으로 빠져나가지 않고 온전히 도형 안을 지나갑니다. 이게 수학에서 말하는 볼록 집합의 정의입니다.

<b> 볼록 집합이 아닌 것들 (움푹 들어간 모양) </b >  
하트 모양, 별 모양, 초승달 모양, 알파벳 'C'나 'L' 모양

특징: 하트 모양을 떠올려 보세요. 하트의 왼쪽 볼록한 윗부분에 점을 하나 찍고, 오른쪽 볼록한 윗부분에 점을 하나 찍은 뒤, 두 점을 자로 곧게 이어보세요.  선 가운데 부분이 하트의 가운데 파인 곳을 지남 -> 볼록집합 아님.

#### Convex Hull
어떤 점들의 집합 $S$가 있을 때, $S$에 속한 모든 점을 포함하는 가장 작은 볼록 집합(Convex Set)
...
신호를 주파수 성분으로 분해한다는 뜻은 복잡한 신호를 "일정한 주기로 출렁이는 가장 기본적이고 규칙적인 신호(주파수)들이 각각 얼마만큼의 비율로 섞여 있는가?"로 쪼개서 분석하는 것을 의미합니다.
...
### Bayes Risk
통계적 결정 이론 및 베이지안 추론에서 특정한 추정 방식(추정량)을 사용했을 때 발생할 것으로 예상되는 '평균적인 손실(Expected Loss)'을 의미합니다. 쉽게 말해, 우리의 추측이 실제 값과 얼마나 빗나갈지를 확률적으로 계산한 평균 페널티 점수라고 볼 수 있습니다.

### 시계열 데이터 (Time Series Data)
시계열 데이터는 연속적인 시간 지점에서 수집되거나 기록된 일련의 데이터 점입니다. 시계열 데이터는 센서 데이터가 근본적으로 시계열인 자동차 분야, 해양 측정 및 기후 분석과 같은 데이터의 과학 연구 분야, 주가 및 기타 지표 등이 있는 금융 분야에서 특히 널리 사용되고 있습니다. 

#### 시계열 데이터 특징
시계열 데이터에는 시간적 요소, 즉 시간이 관련됩니다. 핵심적인 사항은 시간 경과에 따라 순차적으로 수집되거나 기록된 데이터 점이 연속해 있다는 것입니다. 이는 매분, 매초 또는 매일 등 일정 간격을 의미하는 경우가 많지만, 규칙성이 엄격하게 요구되는 것은 아니며 타임스탬프가 있는지 여부가 중요합니다. 여러분의 데이터셋에 타임스탬프가 포함되어 있으며 시간을 기준으로 한 정렬이나 분석이 필요하다면, 이는 시계열 데이터일 가능성이 높습니다.

#### 시계열 데이터의 분석 방법
시계열 데이터를 사용한 작업의 두 가지 핵심 요소는 동기화와 재추출입니다. 동기화에는 타임스탬프에 따라 데이터 점을 정렬하여 데이터셋이 비교 가능하도록 하고 분석이 정확하도록 보장하는 작업이 포함됩니다. 재추출은 특정 분석 목표를 달성하기 위해 데이터 점의 빈도를 조정하는 데 필수적인 작업입니다.
