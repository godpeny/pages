...
두 분포의 의미를 예로 보면 다음과 같습니다.
- $p(y=1 \mid \mathbf{x})$ — **진짜 정답.** 이 클릭이 실제로 전환할 확률입니다. 같은 특성의 클릭 1000개 중 100개가 결국 전환한다면 10%입니다. 아무도 이 값을 직접 알 수 없습니다. 모델이 도달해야 할 목표입니다.
- $b(y=1 \mid \mathbf{x})$ — **우리 스트림에 찍힌 정답.** 같은 1000개 클릭이 학습 데이터에는 이렇게 들어옵니다. 1000개 전부 먼저 0으로 들어오고, 전환한 100개가 나중에 1로 한 번 더 들어옵니다. 총 1100행 중 1이 100행이니 9.1%입니다. 진짜는 10%인데 데이터에는 9.1%로 찍혀 있습니다. 이것이 편향 분포 $b$ 이고, 식으로 쓰면 $b = \frac{p}{1+p}$ 입니다.
...
$$
- $f_\theta(\mathbf{x})$ — 모델의 답. 학습이 끝났을 때 모델이 내놓는 pCVR입니다.
...
### 문제점
시간 지연(Time Delay): 실제 광고 환경에서는 사용자가 광고를 클릭한 시점과 실제로 구매(전환)를 완료하는 시점 사이에 상당히 긴 시간 차이가 존재합니다. 이 지연 때문에 학습 데이터를 수집하는 시점에는 아직 전환이 발생하지 않았지만 나중에는 전환할 예정인 긍정(Positive) 사례들이, 학습 데이터 수집 단계에서는 아직 전환되지 않았다는 이유로 부정(Negative) 사례로 잘못 분류됩니다.

피드백 시프트(Feedback Shift): 학습 데이터와 실서비스 환경에서 유입되는 사용자 분포($P(X)$)는 똑같습니다. 하지만 지연된 피드백 오류 때문에, 동일한 유저($X$)에 대해 학습 데이터가 가리키는 전환 확률 $P(Y|X)$가 실제 서비스에서 일어나야 할 진짜 확률 $P(C|X)$보다 왜곡되어 다르게 나타나는 현상을 의미합니다.
