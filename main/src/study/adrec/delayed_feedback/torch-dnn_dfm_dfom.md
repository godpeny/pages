...
## mtlfnw
이 loss에 들어오는 행(row)은 두 종류뿐입니다.
- y=0 행:  "방금 클릭 발생" (전환 여부 아직 모름 → 일단 네거티브)
- y=1 행:  "아까 그 클릭, 전환됐어!" (정정 신호, action_type이 종류를 알려줌)

```yaml
(1 - y_hat_all_d) * (1 + y_hat_all_d) * log(1 - y_hat)      # 를 K개 head 각각에 → .sum(1)
```
loss는 항이 두 개이고, y가 스위치입니다.

```
output = (1 - y) × [네거티브 항]     ← y=0이면 켜지고, y=1이면 0이 되어 꺼짐
       +    y    × [포지티브 항]     ← y=1이면 켜지고, y=0이면 0이 되어 꺼짐
```
수식과 예로 살펴봅니다.
```
벌점 = (1 - pₖ²) × log(1 - pₖ)
...
클릭 행 하나, K=5 head의 예측:
  y_hat = [0.001, 0.030, 0.008, 0.002, 0.019]

head 1 (구매, p=0.030):  벌점 = (1-0.030²) × log(1-0.030) = 0.9991 × (-0.0305)
head 4 (카트, p=0.019):  벌점 = (1-0.019²) × log(1-0.019) = 0.9996 × (-0.0192)
... (5개 head 전부 각자 계산해서 합산)
```

```
보상 = (1 + pₖ) × log(pₖ)          (k = action_type이 지목한 head만)
        └──┬──┘    └───┬───┘
        이자율      보통의 BCE 포지티브 보상

가입 전환 정정 행 (action_type=3):
  y_hat = [0.001, 0.030, 0.008, 0.002, 0.019]
                              └── gather → p₃ = 0.002

head 3:          보상 = (1+0.002) × log(0.002)     ← 이 head만
head 0,1,2,4:    계산 없음 (네거티브 항은 y=1이라 스위치가 꺼짐,
                            포지티브 항은 gather에 안 뽑힘)
```
...
AdDFOMModel에 MTLDFMModel의 태스크 축(K-head + 마스크)을 이식한, 온라인 멀티태스크 CVR 래퍼입니다.  

MTLDFM보다 훨씬 단출합니다. 이유는 DFOM 철학 때문으로 시간 정보(delay, neg_delay)나 neg_conv 마스크도 필요 없습니다. 추가된 것은 "포지티브를 어느 head로 보낼까"의 action_type 하나뿐입니다.

## 특징
온라인 CVR은 캠페인 목적(objective) 파티션마다 트래픽 규모가 달라, 파티션별로 다른 네거티브 샘플링을 걸 수 있게 설계되어 있습니다. 그래서 평가 보정도 샘플별로 "그 샘플의 목적에 해당하는 비율"을 사용합니다.

라벨 쪽에도 마스크를 겁니다. MTLDFM은 예측만 output_mask로 접었는데, 이 파일은 평가 시 정답 y에도 같은 마스크를 적용합니다. 이유는 데이터가 다르기 때문입니다.   
- MTLDFM (배치): 평가 데이터가 배치 파이프라인에서 만들어진 스냅샷이라, 캠페인 목적과 라벨의 정합이 데이터 생성 단계에서 이미 정리되어 온다고 전제합니다. 래퍼는 접을 필요가 없습니다.
- MTLDFOM (온라인 스트림): 스트림에는 모든 종류의 전환 정정 신호가 섞여 흘러옵니다. SIGN_UP 캠페인 클릭에 구매 정정 신호가 붙어 올 수 있으므로, 래퍼가 평가 시점에 y × select로 직접 걸러야 예측(목적 기준 확률)과 정답이 같은 질문이 됩니다.
