...
> **TL;DR** — 신규 광고그룹의 pCVR이 안정되기까지 걸리는 시간 $N$ 을 줄인다. 대상은 배치 DFM(PUR)과 온라인 DFOM(PF·APP·MEM)을 쓰는 CVR 모델 6종. **$N$ 곡선을 먼저 그린다** — 아직 한 번도 재지 않았다.
>
> **메인** — 근거가 강하고(Tier 1, Alibaba 배포) 공수가 한 곳에 모이는 것.
> ① **ES-DFM 도입 + DEFER·DEFUSE 확장 비교 실험** (온라인, C1) — 지금 온라인 모델은 클릭이 오면 0초 기다리고 바로 "전환 안 함"으로 학습해 모든 전환이 일단 오답으로 시작한다. ES-DFM은 클릭 후 $e$ 시간 기다렸다 라벨을 붙이고, 창 뒤 전환 확률 $p_{dp}$·진짜 미전환 확률 $p_{rn}$ 헤드로 `_fnw` 가중치를 바꾼다. 그 위에 DEFER(전체 클릭 재투입)와 DEFUSE(가짜 음성 확률 $z$) 확장을 얹어 오프라인 스트리밍 재생으로 한 번에 비교한 뒤 승자를 온라인 A/B로 올린다. 셋이 같은 대기 창 스트림을 쓰므로 스트림 변경은 한 번이다.
> ② **옵티마이저 설정 조정** (양쪽, C3) — 임베딩이 한 스텝에 움직이는 폭이 작으면 신규 그룹이 따라잡는 데 오래 걸린다. 임베딩 학습률·weight decay·Clippy 갱신 상한을 조정한다. 성숙 그룹에도 같이 걸리는 변경이라 제약 지표(캘리브레이션·RIG)를 함께 본다.
> 메인은 C1·C3만 다룬다. 신규 그룹의 첫 몇 시간(C2·C4)은 아래 옵션이 맡는다.
>
> **옵션** — 조건이 붙거나 범위가 좁은 것.
> ③ **group_id 임베딩 초기화** (양쪽, C4) — 새 그룹의 임베딩은 지금 0 근처 랜덤값에서 출발한다. 어느 계정·캠페인 밑인지는 이미 알고 있으므로 **같은 계정·캠페인 형제 그룹 임베딩의 평균**으로 시작시킨다. 신규 그룹에만 닿고 성숙 그룹은 건드리지 않는다. **선행 확인**: 현재 구조에서 이런 초기화가 가능한지.
> ④ **GDFM — 장바구니 등 post-click 행동을 전환의 대리 신호로** (온라인, C1·C2) — 구매는 며칠 걸리지만 장바구니는 몇 분이면 관측된다. CART 액션은 `OBJECTIVE_MAP`에 이미 있는데 `mtldfm_v2`가 자기 action_type 헤드만 갱신해 구매 예측에 닿지 않는다. **선행 확인**: 우리 로그에서 장바구니가 구매를 얼마나 예측하는지($H(y \mid \text{cart})$). 정보가 있으면 연결한다.
>
> **참고** — **ULC** (배치, C1·C2). 미전환마다 "나중에 전환할 확률 $w$"를 보조 모델로 붙여 그 값으로 학습한다. DEFUSE의 $z(x)$ 와 같은 아이디어의 배치판이라, 메인 ①의 DEFUSE 팔 결과가 좋으면 배치 DFM에 올릴 근거가 된다.
