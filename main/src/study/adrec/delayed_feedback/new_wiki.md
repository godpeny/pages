...
신규 광고 그룹의 경우 인덱서가 unknown index로 1번을 할당해서 torch-dnn에 전달하고, 이 값은 늘 이전 run으로 override되고 이전 run들에서도 1번은 늘 신규광고그룹용 unknown_index였으므로 자연스럽게 "신규광고그룹"의 평균에 해당하는 가중치를 갖게 된다.
#### 신규 그룹 임베딩이 겪는 두 상태
...
형제 그룹을 "같은 account_id"로 정의하면 배치 row만으로 매핑이 나온다. 외부 조회가 필요 없다.

상태 2의 "처음 등장" 판별은 만들어야 한다. 가장 빠른 방법은 이전 run optimizer의 `exp_avg_sq`가 0인 행을 찾는 것이다. 한 번도 역전파를 안 받은 행이라 코드 추가 없이 판별된다. 정석은 Sequoia 인덱서 문서에 첫 등장 시각을 실어달라고 요청하는 것이다.
...
배치를 읽으며 {group_id → account_id} dict를 쌓습니다. _count_index가 이미 row마다 feature 값을 훑고 있어 그 자리에 한 줄 추가하면 됩니다.
