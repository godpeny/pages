...
Note that if $A$ is an $m \times n$ matrix that can be row reduced to echelon form without row exchanges, then $A = LU$. Here, $L$ is a lower triangular $m \times m$ matrix with 1's on the diagonal, $U$ is an upper triangular matrix and also echelon form of $A$. Therefore, $A$ and $U$ have same number of pivots.
