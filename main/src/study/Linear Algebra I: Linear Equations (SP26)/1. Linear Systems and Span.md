...
#### EF vs RREF
pivot/independence 찾는 것은 EF에서도 가능하지만 RREF가 더 직관적임. 즉, EF에서 조금만 더 연산하면 무조건 RREF가 된다. 다시말해 모든 행렬은 항상 RREF로 만들 수 있다.

- EF: 중간 단계 (계단만 맞춤)
- RREF: 완전 정리 (pivot 위/아래 다 0)

### Row Reduction을 하는 이유
#### 1.pivot 찾기
rank, independent 등 구조 파악하기 위함.
- EF에서 pivot은  각 행의 첫 번째 nonzero가 pivot 이고 그 열이 pivot column.
- RREF에서 pivot은 각 행의 첫 번째 nonzero 이면서 1이고 그 열이 pivot column.

pivot column의 개수 = independent column의 개수 = rank
하지만 independent하다고 해서 pivot column인 건 아님($c_3 = c_1 + c_2$ 등으로 표현 가능)

#### 2.방정식이 풀리는지 확인
RREF는 방정식을 바꾸는 게 아니라 “같은 문제를 더 단순하게 쓰는 것”.
RREF에서 어떤 행이든 $(000 \mid c), c \neq 0$ 행이 있으면 inconsistent 이고 없으면 consistent 이다.
