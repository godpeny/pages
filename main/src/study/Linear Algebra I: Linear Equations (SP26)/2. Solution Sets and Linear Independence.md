...
Suppose $\vec{v1}, \cdots , \vec{v_k}$ are vectors in $\mathbb{R^n}. If
$k > n$, then $\{\vec{v_1}, \vec{v_2}, \cdots, \vecv{v_k}}$ is linearly dependent. This is because if matrix has more columns than rows, so every column cannot be pivotal.
