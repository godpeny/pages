...
모듈러 연산에서 L2 손실 함수(MSE)는 신경망의 출력값과 실제 정답인 원-핫 인코딩(one-hot encoding) 벡터 사이의 거리를 최소화하는 역할을 합니다. 오리지널 손실함수는 아래와 같은 수식입니다.

$$
\ell = \|Output - one\_hot(c)\|_2^2 = \sum_i \left\| P_{\perp 1} \left( \frac{1}{2d} o[i] - e_{l[i]} \right) \right\|^2 $$

- $o[i]$는 신경망의 예측값, 
- $e_{l[i]}$는 정답(모듈러 연산 결과), 
- $P_{\perp 1}$은 평균을 0으로 만드는 투영 행렬(zero-mean projection matrix)
...
- $-2r_{kkk}$ (정답 신호 항, $R_g$): 모델이 올바른 주파수 응답을 내놓기 위한 조건입니다. 전역 최적화를 위해 합 전위 값 **$r_{kkk}$는 1**이 되어야 합니다.
- $\sum_{k_1, k_2} |r_{k_1k_2k}|^2$ (교차 주파수 간섭 항, $R_c$): 서로 다른 주파수 성분들끼리 부딪히지 않게 하는 조건입니다. 전역 최적해에서 이 값은 **0**이 되어야 합니다.
- $\frac{1}{4} | \sum \sum r_{p,k',-k',k} |^2$ (노이즈 억제 항, $R_n$):불필요한 노이즈 성분을 소거하는 조건입니다. 전역 최적해에서 이 값은 $0$ 이 되어야 합니다.
- $\frac{1}{4} \sum_{m \neq 0} | \sum \sum r_{p,k',m-k',k} |^2$ (조합론적 제약 항, $R^*$): 입력값 결합 시 발생하는 수학적 모순을 해결하는 조건입니다. 전역 최적해에서 이 값은 0이 되어야 합니다.

즉, $-2r_{kkk}$(목표 항)이 우리가 원하는 정답 신호이고 이 값을 최대화(즉, 손실을 최소화)하려면 $r_kkk$ 가 1이 되어야 합니다. 반대로 $\sum_{k_1, k_2} |r_{k_1k_2k}|^2 \cdots \sum_{k'} r_{p,k',m-k',k}$(나머지 항)은 오차(Error)나 노이즈를 발생시키는 주파수 간의 꼬임 현상입니다. 전역 최적해가 되려면 이 복잡한 합들이 모두 0이 되어야 합니다. 가령 $\sum |r_{k_1k_2k}|^2$ 항은 입력 주파수 $k_1, k_2$ 가 출력 주파수 $k$와 맞지 않을 때 발생하는 오차를 나타냅니다.

이 복잡한 식을 경사하강법을 이용해서 미분해서 푸는 대신에 CoGS는 이 손실 함수의 복잡한 조건들을 대수적인 조립(Composing)을 통해 만족 시킬 수 있습니다.  


### 합 전위 (Sum Potentials)
$$r(z)= \sum_j \prod z_{pkj}$$
모든 은닉 노드(j)에 걸쳐 특정 가중치들의 곱(단항식)을 합산한 값.  
e.g., $r_{k_1k_2k}$: $a$의 주파수 $k_1$, 입력 $b$의 주파수 $k_2$, 그리고 출력 $c$의 주파수 $k$에 대응하는 복소수 가중치들을 모든 노드에서 곱하고 더한 값
...
### Conclusion
모듈러 연산의 손실함수는 원래 아주 복잡한 비선형 함수(MSE)이지만 활성화 함수를 이차함수로 쓴다는 제약 아래 "Structure of Loss Functions" 의 수식처럼 SP의 조합으로 분해된 모습으로 재구성 할 수 있습니다. 이 재구성된 손실 함수는 이차 활성화 함수(Quadratic Activation) 조건 하에 합 포텐셜(Sum Potentials, SP)들의 조합으로 재구성됩니다,. 이 수식은 전역 최적해(Global Optimizer)가 되기 위해 만족해야 할 제약 조건(Constraints)들의 목록으로 이루어져 있고, 우리가 찾는 해는 이 연립 방정식을 모두 만족 (r_a=0 or r_b=1) 해야합니다. 
하지만 CoGS는 이 복잡한 조건들을 경사 하강법(Gradient Descent, GD)으로 더듬어가며 찾는 것이 아니라, 대수적으로 조립(Construction)하여 즉시 만들어냅니다. 이 수식이 "Exemplar constructed global optimizers" 의 수식으로, "Structure of Loss Functions"에서 정의된 복잡한 항들을 분석하여, 이에 대응하는 부분 해들을 대수적 연산(곱셈, 덧셈)으로 조립하는 방법으로 경사하강법 없이 부분합의 조합으로 최적화를 할 수 있는 방법이 됩니다.
