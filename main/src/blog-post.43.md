...
- Trino: 대규모 데이터 세트에 대해 고속으로 분산 SQL 쿼리를 실행할 수 있도록 설계된 오픈소스 분산 SQL 쿼리 엔진입니다. 쉽게 말해, 서로 다른 곳에 저장되어 있는 방대한 데이터를 한곳으로 모으지 않고도, 익숙한 SQL 문법을 사용해 마치 하나의 데이터베이스에 있는 것처럼 빠르게 조회(Query)할 수 있게 해주는 기술입니다. 다시 말해, Hadoop(Hive), 분산 오브젝트 스토리지(AWS S3, MinIO 등)에 저장된 데이터 레이크뿐만 아니라, PostgreSQL, MySQL, Oracle 같은 관계형 데이터베이스(RDBMS), NoSQL, Cassandra 등 다양한 데이터 소스를 동시에 연결하여 쿼리할 수 있습니다. 그리고 트리노는 데이터를 직접 저장하지 않는 오로지 '연산(Query Engine)' 전용 시스템입니다. 데이터를 메모리 위에서 파이프라인 방식으로 처리하기 때문에 대용량 데이터 조회 속도가 매우 빠릅니다.
