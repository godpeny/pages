...
## Inference Server (AI 추론 서버)
AI 추론은 AI 모델이 데이터를 기반으로 답변을 제공하는 것을 말합니다. 추론 단계에서 모델은 학습한 내용을 실제 상황에 적용할 수 있습니다.성공적으로 추론하려면 AI 모델이 단기간에 많은 연산을 수행해야 합니다. 따라서 모델 규모, 높은 사용자 볼륨, 대기 시간과 같은 요인은 모두 성능을 제한할 수 있습니다. 모델에 필요한 데이터와 메모리가 늘어날수록 하드웨어와 가속기가 따라잡기 어렵습니다.

AI 추론 서버는 AI 모델이 학습 단계에서 운영 단계로 넘어갈 수 있게 도와주는 소프트웨어입니다. 머신 러닝을 통해 모델이 학습한 내용을 실제에 적용하여 추론을 생성하도록 돕습니다. 효율적인 결과를 얻기 위해서는 AI 추론 서버와 AI 모델이 호환되어야 합니다. 다음은 추론 서버와 각 추론 서버에 가장 적합한 모델의 예시입니다.

- 멀티모달 추론 서버: 이 유형의 추론 서버는 한 번에 여러 모델을 지원할 수 있습니다. 데이터를 코드, 이미지 또는 텍스트 형식으로 수신한 다음 단일 서버에서 모든 추론을 처리합니다. 멀티모달 추론 서버는 여러 개의 모델을 지원하기 위해 GPU 및 CPU 메모리를 더욱 효율적으로 사용합니다. 이를 통해 하드웨어를 간소화하여 확장을 용이하게 하고 비용 최적화를 지원합니다.
- 단일 모델 추론 서버: 이 추론 서버는 여러 모델이 아닌 하나의 모델만 지원합니다. 여기서는 AI 추론 프로세스도 특정 활용 사례를 기반으로 학습한 모델과 커뮤니케이션하도록 특화되어 있습니다. 이 유형의 서버는 텍스트 또는 코드 형식의 데이터만 처리할 수 있습니다. 특정 분야에 집중되어 있어 효율성이 매우 높기 때문에 실시간으로 의사결정을 내리거나 리소스가 제한적인 경우에 유용합니다. 

### AI 추론의 여러 유형
다양한 유형의 AI 추론을 사용하여 다양한 활용 사례를 지원할 수 있습니다.

- 배치 추론: 배치 추론은 대규모 그룹에서 데이터를 수신하고 처리하는 방식에서 이름이 유래되었습니다. 이 방법은 추론을 실시간으로 처리하는 대신 정기적으로 데이터를 처리하며, 데이터의 양과 AI 모델의 효율성에 따라 그 간격은 매시간 또는 매일일 수도 있습니다. 이러한 추론을 '오프라인 추론' 또는 '정적 추론'이라고도 합니다.
- 온라인 추론: 온라인 추론, 즉 '동적' 추론을 통해 실시간으로 대응할 수 있습니다. 이러한 추론은 대기 시간을 줄이고 고속 예측을 지원하는 하드웨어와 소프트웨어를 필요로 합니다. 온라인 추론은 엣지에서 유용할 수 있습니다. 즉 데이터가 있는 위치에서 AI가 작업을 수행하는 것을 의미합니다. 위치 예시로는 휴대폰, 자동차 또는 연결이 제한된 원격 사무실 등이 있습니다.
e.g., OpenAI의 ChatGPT는 온라인 추론의 좋은 예입니다. ChatGPT는 빠르고 정확한 답변을 제공하기 위해 사전에 대규모의 운영 지원을 필요로 합니다.
- 스트리밍 추론: 스트리밍 추론은 사람과 통신하는 데 반드시 사용되는 것은 아닌 AI 시스템을 의미합니다. 이 모델은 프롬프트와 요청을 사용하는 대신 상시 데이터 흐름을 수신하여 예측을 수행하고 내부 데이터베이스를 업데이트합니다. 스트리밍 추론은 변화를 모니터링하거나 규칙성을 유지하거나 문제를 사전에 예측하는 데 활용할 수 있습니다. 

### NVIDIA Triton Inference Server
AI 모델의 추론(Inference) 과정을 간소화하고 최적화하도록 설계된 오픈소스 추론 서빙 소프트웨어입니다. AI 모델을 배포할 때, 모델 개발에 사용된 다양한 프레임워크와 하드웨어 환경의 파편화로 인해 관리가 복잡해집니다. Triton은 이러한 복잡성을 해결하여, 개발자가 어떤 프레임워크로 모델을 만들었든, 어떤 하드웨어 환경이든 관계없이 효율적으로 AI 서비스를 제공할 수 있도록 돕습니다.

#### Architecture
Triton Inference Server는 클라이언트 요청부터 하드웨어 가속기 활용까지의 과정을 고도로 모듈화하여 처리합니다. 대략적인 흐름과 핵심 구성 요소는 다음과 같습니다.

<b> 1. Model Repository (모델 저장소) </b>  
Triton이 서빙할 AI 모델들이 저장되는 파일 시스템 기반의 공간입니다. 로컬 디렉터리뿐만 아니라 AWS S3, Google Cloud Storage 같은 클라우드 스토리지도 지정할 수 있습니다. 각 모델 폴더 안에는 모델의 실제 가중치 파일과 함께 버전 정보, 입력/출력 텐서의 형태(Shape) 및 데이터 타입을 정의한 모델 설정 파일(config.pbtxt)이 포함됩니다.

<b> 2. Client API & Protocols (입력 엔드포인트) </b>  
외부 클라이언트가 Triton에 추론 요청을 보낼 때 사용하는 인터페이스입니다. HTTP/REST 및 gRPC 로 커뮤니티 표준인 KServe 프로토콜(쿠버네티스(Kubernetes) 기반의 AI 모델 서빙 플랫폼인 KServe 프로젝트에서 제안하고, 현재 AI 업계의 표준으로 자리 잡은 클라이언트와 추론 서버 간의 통신 규약)을 기반으로 고성능 통신을 지원합니다. 일반적으로 대용량 스트리밍이나 빠른 응답이 필요할 때는 gRPC가 선호됩니다. 
- In-Process 지원: In-Process(내장형 라이브러리) 형태로 구동한다는 것은, Triton을 별도의 독립된 ‘서버 프로그램’으로 띄우지 않고 내가 만드는 프로그램(애플리케이션) 내부에 하나의 소스코드 라이브러리처럼 포함시켜 호출한다는 뜻입니다. 이를 통해 린 Triton의 아키텍처(동적 배칭, 스케줄러, 멀티 프레임워크 백엔드)를 그대로 사용하지만 HTTP나 gRPC 같은 네트워크 프로토콜 스택을 거치지 않고 포인터 기반 데이터 전달을 할 수 있습니다. 즉, 동일한 메모리 주소 공간을 쓰기 때문에, 앱이 가지고 있는 입력 데이터(이미지 픽셀 데이터 등)의 메모리 주소(Pointer)를 Triton 라이브러리에 그대로 넘겨줄 수 있습니다. 데이터의 복사(Copy) 오버헤드가 거의 제로($0$)에 수렴합니다. (C++, Java API 지원)

<b> 3. Per-Model Scheduler & Batcher (스케줄러 및 배치 처리) </b>  
요청이 들어오면 Triton은 각 모델에 지정된 독자적인 스케줄러로 요청을 라우팅합니다. Triton의 핵심 성능 비결이 바로 이 강력한 스케줄링 알고리즘에 있습니다. 
- 동적 배치(Dynamic Batching): 서로 다른 시점에 들어온 개별 추론 요청들을 서버단에서 하나의 배치(Batch)로 묶어 GPU에 전달합니다. GPU의 병렬 처리 능력을 극대화하여 처리량(Throughput)을 크게 높입니다.
- 시퀀스 배치(Sequence Batching): 챗봇(LLM)이나 음성 인식처럼 이전 상태(State) 유지가 필요한 상태 중심(Stateful) 모델을 위해, 요청 간의 인과관계를 유지하며 배칭을 수행합니다.
- 동시 모델 실행(Concurrent Model Execution): 하드웨어 자원이 허용하는 한, 하나의 GPU 내에서 여러 모델(또는 동일 모델의 여러 인스턴스)을 동시에 실행하여 자원 고갈이나 대기 시간을 최소화합니다.

<b> 4. Backends (백엔드 실행부) </b>  
스케줄러에 의해 정렬된 배치 요청은 해당 모델 타입에 맞는 백엔드(Backend)로 전달됩니다. 백엔드는 실제로 모델을 구동하여 연산을 수행하는 역할을 합니다. Triton은 플러그인 형태의 Backend C API를 제공하므로, 필요에 따라 사용자가 직접 전/후처리(Pre/Post-processing) 로직이나 새로운 딥러닝 프레임워크를 커스텀 백엔드로 확장하여 붙일 수 있습니다.

<b> 5. Model Pipelines (모델 파이프라인 관리)</b>  
복잡한 AI 서비스는 하나의 모델로 끝나지 않는 경우가 많습니다. (예: 오디오 입력 -> 음성 인식 모델 -> 번역 모델 -> 음성 합성 모델) Triton은 이를 효율적으로 연결합니다. 
- Ensemble (앙상블): 데이터 전처리, 모델 A, 모델 B, 후처리 단계를 데이터 복사 오버헤드 없이 메모리 상에서 파이프라인으로 엮어 실행합니다.
- BLS (Business Logic Scripting): 파이썬 스크립트를 이용해 조건문(if-else)이나 반복문 등을 포함한 복잡한 비즈니스 로직 기반의 모델 호출 흐름을 다이내믹하게 제어할 수 있습니다.

요약하자면, Triton Inference Server는 "클라이언트 요청 수신 $\rightarrow$ 스마트한 동적 배칭 및 스케줄링 $\rightarrow$ 프레임워크별 백엔드를 통한 하드웨어 가속 추론 $\rightarrow$ 결과 반환" 구조를 유기적이고 고성능으로 처리해 주는 AI 배포의 핵심 허브라고 볼 수 있습니다.
...
```yaml
            ...
            │                         │
            ▼                         ▼
  POST /features/creative/    POST /inference
       embedding                      │
            │                         │
            └─────────┬───────────────┘
                      ▼
        ...
        ┌─────────────┴─────────────┐
        ▼                           ▼
  curas-embedder                inf-dev
  (Embedding Triton)                 (pCTR Triton)
  
# 임베딩/유저 피쳐 조회 후 Triton에 값 전달하는 흐름은 서빙팀이 수행

- Embedding Triton: s-bert 모델을 "소재 파싱 + 토크나이저 + s-bert"로 묶은 앙상블 파이썬 백엔드로 Triton에 올라가 있음. 서빙팀이 준비/운영하며, 모델이 고정되어 있어 실험이 불가능.
- pCTR Triton: 실험 설정에서 mlflow에 올라간 모델을 선택하면, MLOps팀이 해당 모델을 준비/서빙. 서빙팀이 ①의 임베딩 값과 유저 피쳐를 조회해 Triton에 넘겨 pCTR을 산출
```
1. curas-scorer는 두 Triton에 양방향으로 붙는 단일 진입 서비스 — 임베딩과 pCTR 둘 다 책임
2. triton-ko-sbert는 임베딩 전용 Triton(curas-embedder) — 처음 설명의 ①번 그 자체
3. pCTR(②번)은 별도 Triton(inf-dev)에서 운영 — 이 디렉토리와 무관
4. global_24_2는 코드상 미사용 — 분리 이전의 잔재로 보이며 정리 가능 후보
5. 임베딩 = 코드 하드코딩(실험 불가), pCTR = request param 동적(실험 가능)
6. 캐싱 없음 — 두 경로 모두 매 호출 Triton 직격 (임베딩의 사전 계산은 curas-scorer가 아니라 호출자/저장소 측 책임)

##### Embedding Triton 상세
<b> Embedding Triton(ko-sbert ensemble) 두 경로 </b>  

``` yaml
# 경로 A — 일반 텍스트 (`ko-sbert-onnx-mt-ensemble`)

요청 → ensemble scheduler
         │
         ├─ ko-sbert-mt-tokenizer 호출 → (default scheduler → python backend)
         │
         └─ ko-sbert-onnx-mt 호출      → (default scheduler → onnxruntime backend)
       ↓
     응답
```

```yaml
# 경로 B — 채팅버블 JSON (`ko-sbert-mt-parsing-ensemble`)
요청 → ensemble scheduler
         │
         ├─ ko-sbert-mt-parsing-tokenizer 호출 → (default scheduler → python backend)
         │
         └─ ko-sbert-onnx-mt 호출              → (default scheduler → onnxruntime backend)
       ↓
     응답
...
<b> 스케줄러 정리 </b>  
Triton 에서는 config.pbtxt에 dynamic_batching {}이나 sequence_batching {} 블록이 없으면 자동으로 default scheduler가 됩니다.

- ko-sbert-onnx-mt/config.pbtxt: 두 블록 다 없음 → default
- tokenizer: 두 블록 다 없음 → default
그래서 ensemble을 제외한 모든 모델이 default scheduler입니다.

| Triton 컴포넌트 | 이 시스템 구현 |
|---|---|
| **Client** | curas-scorer (Go, HTTP/REST) |
| **Inference Protocol** | KServe v2 over HTTP |
| **Model Repository** | `/share-vol` ← S3에서 init container 동기화 |
| **Model Management** | explicit 모드, 7개 모델 명시 로드 |
| **Scheduler** | Default + Ensemble (Dynamic/Sequence batcher 미사용) |
| **Backend** | onnxruntime + python (custom 전처리) |
| **Instances** | 모델당 1, 파드 2 replicas로 수평 확장 |
| **Health/Metrics** | k8s probe + Prometheus metrics |
| **Hardware** | CPU 노드(b2 node-group), GPU 미사용 |

**가장 핵심적으로 활용되는 Triton 기능 두 가지**:
...
## Pacing(페이싱)
광고 총 지출이 전적으로 경매 시장의 트래픽 양에 의존하지만 광고주들은 예산 한도 제약을 가지고 있습니다. 따라서 낙찰된 모든 광고의 누적 지출액이 이 예산 한도를 초과하지 않고 계획대로 잘 소진되도록 제어할 필요가 있으므로 페이싱의 도입이 필요합니다.

### 할인 페이싱
예산 제약을 적용하기 위해 지출 속도를 실질적으로 조절하는 수학적/시스템적 메커니즘을 설명합니다.
최종 입찰가에 페이싱 제어 변수를 곱하여 새로운 입찰가를 계산합니다.

조정된 최종 입찰가 = 페이싱 제어 변수 * 최종 입찰가

여기서 페이싱 제어 변수 는 0보다 크고 1 이하의 값을 가지는 변수로, 이러한 방식을 '할인 페이싱(discount pacing)' 이라고 합니다. 페이스를 늦추고 싶으면 람다를 줄여서 입찰가를 줄여 낙찰될 확률을 줄여서 늦추고, 페이스를 올리고 싶으면 반대로 람다를 높여서 입찰가를 높여 확률을 높이는 방식입니다.

실제 경매환경에서는 이 페이싱 제어변수를 미세하게만 조정해도 전체 지출 속도에 매우 큰 변화(disproportionately large changes)가 일어나기에 목표 작동 지점 근처에서 조정 폭을 명시적으로 제한할 수 있는 비선형 제어 전략이 반드시 필요하다고 강조합니다.

### Pacing Controller
기존의 페이싱 제어기의 작동 방식을 설명합니다.
기존 시스템은 최근 지출 추세의 안정성(변동 계수, F)을 평가한 뒤, 목표 지출 속도에 도달하기 위해 페이싱 제어 변수의 조정 폭(α)을 일정한 계단식 단계로 높이거나 낮추며 제어하는 방식입니다.

#### 변동 계수, F
'누적된 총 변화량'의 처음 관측값(시작점)과 가장 최근 관측값(끝점) 사이의 직선적인 차이

- F≈1일 때: 람다가 낭비 없이 한 방향으로 부드럽고 일관되게 움직이고 있음을 의미합니다.
- F≫1일 때: 람다가 위아래로 심하게 요동치며 불안정한 상태임을 의미합니다.

Reference: https://arxiv.org/pdf/2509.25429v2
